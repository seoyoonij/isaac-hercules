...
from pegasus.simulator.logic.rotations import rot_ENU_to_NED, rot_FLU_to_FRD, usd_body_prim_frame_is_frd
...
class Magnetometer(Sensor):
    ...
    def update(self, state: State, dt: float):
        ...
        # Rotate the inertial field into the FRD body frame. ``rot_body_to_world`` must map
        # body-FRD → **NED** so ``.inv()`` matches the NED quaternion sent to ArduPilot / PX4.
        # FRD USD body: ``E * R_body_enu`` (no FLU→FRD). FLU USD body: ``E * R_flu * F^{-1}``.
        if usd_body_prim_frame_is_frd():
            rot_body_to_world = rot_ENU_to_NED * attitude_flu_enu
        else:
