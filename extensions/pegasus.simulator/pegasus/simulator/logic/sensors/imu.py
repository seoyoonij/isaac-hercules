...
from pegasus.simulator.logic.rotations import (
    angular_velocity_to_frd_for_publish,
    attitude_ned_frd_quat_from_body_to_enu,
    linear_acceleration_to_frd_for_publish,
)
...
class IMU(Sensor):
    ...
    def update(self, state: State, dt: float):
        ...
        # Convert the orientation to the FRD-NED standard (see rotations.usd_body_prim_frame_is_frd)
        attitude_frd_ned_q = attitude_ned_frd_quat_from_body_to_enu(state.attitude)

        # Convert the angular velocity from FLU to FRD standard (no-op when USD body is already FRD)
        angular_velocity_frd = angular_velocity_to_frd_for_publish(angular_velocity)

        # Convert the linear acceleration in the body frame from FLU to FRD standard
        linear_acceleration_frd = linear_acceleration_to_frd_for_publish(linear_acceleration)

        # Add the values to the dictionary and return it
        self._state = {
            "orientation": attitude_frd_ned_q,
