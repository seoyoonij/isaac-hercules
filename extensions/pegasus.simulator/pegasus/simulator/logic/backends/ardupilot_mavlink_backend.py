...
class ArduPilotMavlinkBackendConfig(BackendConfig):
    ...
    def __init__(self, config={}):
        ...
        self.connection_baseport = config.get(
            "connection_baseport", ARDUPILOT_SIM_MAVLINK_BASEPORT
        )
        # Base port must not overlap sim_vehicle MAVProxy GCS UDP (14550 + 10*instance);
        # see ARDUPILOT_SIM_MAVLINK_BASEPORT in params.py.
...
class ArduPilotMavlinkBackend(Backend):
    ...
    def __init__(
        ...
        self._connection_baseport = config.connection_baseport
    ...
    def start(self):
        ...
        if self.ardupilot_autolaunch and self.ardupilot_tool is None:
            ...
            self.ardupilot_tool = ArduPilotLaunchTool(
                ardupilot_dir=self.ardupilot_dir,
                vehicle_id=self._vehicle_id,
                ardupilot_vehicle=self.ardupilot_vehicle,
                ardupilot_model=self.ardupilot_vehicle_model,
                mavlink_baseport=self._connection_baseport,
            )
    ...
    def update_motor_commands(self, servos):
        ...
        elif self._armed and len(servos) == 0:
            carb.log_warn(f"Zero servo input while armed! Servos: {servos}")
