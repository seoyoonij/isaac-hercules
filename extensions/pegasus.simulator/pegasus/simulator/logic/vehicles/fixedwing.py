...
class FixedWingConfig:
    ...
    def __init__(self):
        ...
        # DOF name passed to DynamicControl.find_articulation_dof (often matches the RevoluteJoint prim name)
        ...
        # Nested USD: /body under spawn + inner Xform (e.g. defaultPrim "Shahed" -> usd_inner_prefix="Shahed").
        self.usd_inner_prefix = ""
        # PhysX articulation root under _asset_root (e.g. "body" when ArticulationRootAPI is on body, else "").
        self.robot_articulation_suffix = ""
...
class FixedWing(Vehicle):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config.graphs,
            config.backends,
            usd_inner_prefix=config.usd_inner_prefix,
            robot_articulation_suffix=config.robot_articulation_suffix,
        )
    ...
    def _get_vehicle_mass_kg(self):
        ...
        try:
            body_prim = self._world.stage.GetPrimAtPath(self._asset_root + "/body")
    ...
    def _resolve_propeller_joint(self):
        ...
        # Pegasus fixed_wing.usd: articulation often at /propeller; custom rigs may use /body or robot root.
        candidate_paths = (
            self._asset_root + "/propeller",
            self._asset_root + "/body",
            self._robot_prim_path,
            self._asset_root,
        )
        seen = set()
        for apath in candidate_paths:
            if apath in seen:
                continue
            seen.add(apath)
            if not stage.GetPrimAtPath(apath).IsValid():
                continue
            try:
                articulation = self.get_dc_interface().get_articulation(apath)
            except RuntimeError:
                continue
            ...
            try:
                joint = self.get_dc_interface().find_articulation_dof(
                    articulation, self._propeller_joint_name
                )
            except RuntimeError:
                continue
            ...
            if joint is not None and joint != invalid_handle:
                ...
                carb.log_info(
                    f"FixedWing propeller visual joint '{self._propeller_joint_name}' "
                    f"via articulation at '{apath}'"
                )
                return

        self._prop_joint_resolved = True
        carb.log_warn(
            f"FixedWing propeller visual joint '{self._propeller_joint_name}' not found "
            f"(tried under asset root {self._asset_root} and robot prim {self._robot_prim_path}). "
            "Skipping visual spin."
        )
