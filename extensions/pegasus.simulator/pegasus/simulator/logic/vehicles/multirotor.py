...
class MultirotorConfig:
    ...
    def __init__(self):
        ...
        # See Vehicle: nested USD (e.g. inner defaultPrim name) and PhysX articulation root under that subtree.
...
class Multirotor(Vehicle):
    ...
    def update(self, dt: float):
        ...
        articulation = self.get_dc_interface().get_articulation(self._robot_prim_path)
    ...
    def force_and_torques_to_velocities(self, force: float, torque: np.ndarray):
        ...
        rb = self.get_dc_interface().get_rigid_body(self._asset_root + "/body")

        # Get the rotors of the vehicle
        rotors = [
            self.get_dc_interface().get_rigid_body(self._asset_root + "/rotor" + str(i))
